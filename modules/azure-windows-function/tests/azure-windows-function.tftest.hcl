...
run "should_create_function_app_with_reasonable_defaults" {
  ...
  assert {
    condition     = length(azurerm_windows_function_app_slot.slot) == 0
    error_message = "Should not create app slot"
  }

  assert {
    condition     = azurerm_windows_function_app.app.name == "tst-tdd-fn-myFunc"
    error_message = "Should assign default name to function app"
  }

  assert {
    condition     = azurerm_windows_function_app.app.resource_group_name == var.resource_group_name
    error_message = "Should associate with the correct resource group"
  }

  assert {
    condition     = azurerm_windows_function_app.app.https_only == true
    error_message = "Should require https only"
  }

  assert {
    condition     = azurerm_windows_function_app.app.enabled == true
    error_message = "Should be enabled automatically"
  }

  assert {
    condition     = azurerm_windows_function_app.app.location == var.location
    error_message = "Should set in correct location"
  }

  assert {
    condition     = azurerm_windows_function_app.app.functions_extension_version == "~4"
    error_message = "Should set as version 4 of functions extension"
  }

  assert {
    condition     = azurerm_windows_function_app.app.public_network_access_enabled == true
    error_message = "Should enable public access"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].minimum_tls_version == "1.2"
    error_message = "Should set minimum TLS version to 1.2"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].use_32_bit_worker == false
    error_message = "Should use 64 bit worker by default"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].websockets_enabled == false
    error_message = "Should not enable websockets by default"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].http2_enabled == true
    error_message = "Should enable http2 by default"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].ftps_state == "Disabled"
    error_message = "Should not enable ftps by default"
  }

  assert {
    condition     = azurerm_windows_function_app.app.site_config[0].application_insights_key == var.app_insights_key
...
run "should_create_function_app_slot_with_reasonable_defaults" {
  ...
  assert {
    condition     = length(azurerm_windows_function_app_slot.slot) == 1
    error_message = "Should create app slot"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].name == "staging"
    error_message = "Should assign default name 'staging' to slot"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].https_only == true
    error_message = "Should require https only"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].enabled == true
    error_message = "Should be enabled automatically"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].functions_extension_version == "~4"
    error_message = "Should set as version 4 of functions extension"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].public_network_access_enabled == true
    error_message = "Should enable public access"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].site_config[0].minimum_tls_version == "1.2"
    error_message = "Should set minimum TLS version to 1.2"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].site_config[0].use_32_bit_worker == false
    error_message = "Should use 64 bit worker by default"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].site_config[0].websockets_enabled == false
    error_message = "Should not enable websockets by default"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].site_config[0].http2_enabled == true
    error_message = "Should enable http2 by default"
  }

  assert {
    condition     = azurerm_windows_function_app_slot.slot[0].site_config[0].ftps_state == "Disabled"
    error_message = "Should not enable ftps by default"
  }
}
